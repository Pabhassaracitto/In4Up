...
| AI-CHAT-01 | Chat: báo "Chưa nạp model AI" sau khi gửi + nút gửi xoay vòng mãi | 🔄 doing (v2: maxTokens 320 + template gemma-3 — chờ chủ build DEV) | timeout firing đúng (build 102978f); v2: 320 tokens + 4 phút + EOS sớm + rescue summary |
...
- **Lịch sử:**
  ...
  - 2026-08-29 | done→done (v2 hiệu chỉnh cho máy yếu) | owner + agent arena/01a02a4a-in4up | Chủ build DEV `102978f` (thâu hoạch cherry-pick e2004cc4+b13f8319 của 01a02a4a + fix 1a95a4ff) — quan sát: xoay vòng xong hiện "AI xử lý quá lâu (model lớn trên máy yếu). Vui lòng thử lại sau vài giây." = **timeout 3 phút firing đúng thiết kế** (không còn treo vô hạn — fix 1 hoạt động), nhưng tablet của chủ chạy ~1.5–2 token/s ⇒ 512 tokens > 3 phút. HIỆU CHỈNH v2: (1) `maxTokens 512→320` (đủ schema chat thông thường; JSON bị cắt thì `_rescueSummary` cứu) + `timeout 3→4 phút`; (2) prompt conversation bọc theo **chat template gemma-3** (`<start_of_turn>user…<end_of_turn><start_of_turn>model`) — model được train để kết thúc turn bằng EOS khi trả lời ĐỦ ⇒ sinh 100–200 token (~1–2 phút) thay vì chạy hết trần 320 (~3.5 phút); (3) request mới trong lúc native vẫn đang sinh tiếp request trước (sau timeout, không cancel được FFI): chờ 60s rồi báo "AI vẫn đang xử lý tin nhắn trước đó — thử lại sau vài giây" thay vì chen request 2. Đồng bộ: branch 01a02a4a adopt bản `addOnExitListener(SendPort, {response})` của 251e (1a95a4ff) — bản closure của 01a02a4a lỗi compile trên SDK CI.
