...
class MemoryLifecycleEngine {
  ...
  void recordImplicit(LearningAction action) {
    switch (action.actionType) {
      case LearningActionType.savedToWordlist:
      case LearningActionType.sentToWriting:
        throw ArgumentError(
            'Action người dùng (${action.actionType.name}) phải đi qua '
            'promote() — không được recordImplicit (mục 2.5 + mục 6).');
      case LearningActionType.opened:
        _bumpReopen(action);
        break;
      case LearningActionType.replayed:
        _bumpReplay(action);
        break;
      case LearningActionType.highlighted:
        _flagHighlight(action);
        break;
      case LearningActionType.translated:
        _flagTranslate(action);
        break;
      case LearningActionType.chatAsked:
      case LearningActionType.skipped:
        // Chỉ phục vụ Attention Score (mục 5) — không tác động lifecycle.
        return;
    }
