...
///    migration (ngữ nghĩa giữ nguyên).
...
///    nằm nguyên trong box cũ; migrator chỉ LIỆT KÊ vào report
///    ("không mất âm thầm").
///
/// LƯU Ý KỸ THUẬT (postmortem bisect B1–B7, 2026-08-20):
///  KHÔNG import trực tiếp `skill_review_data.dart` trong file này —
///  kết hợp import trực tiếp + import `word_entry.dart` (vốn export
///  chính file đó) làm analyzer gãy ở CI. Đọc dữ liệu skill qua FIELD
///  của WordEntry (`entry.understandData.easeFactor`…) — vừa sạch hơn
///  về phụ thuộc, vừa tránh đúng chỗ gãy đã được bisect chứng minh.
/// ═══════════════════════════════════════════════════════════════
library;
...
class WordEntryMigrator {
  ...
  static final RegExp _mmss = RegExp(r'^(\d{1,2}):(\d{2})(?::(\d{2}))?$');
  ...
  /// (nextReview null = "due ngay" ⇒ dùng mốc migration).
  ...
  static EvidenceLocator _toLocator(VocabContext ctx) {
    ...
    final isWebLike = ctx.sourceType == 'web' ||
        refType == 'webUrl' ||
        ctx.sourceType == 'youtube';
  ...
  /// "02:15" → 135 giây; "1:02:15" → 3735 giây. Không parse được ⇒ null.
