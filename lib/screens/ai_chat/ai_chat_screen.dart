...
class _AiChatScreenState extends State<AiChatScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('I2U AI Chat'),
        backgroundColor: const Color(0xFF11162A),
        actions: [
          Consumer<AiServiceFacade>(
            builder: (context, facade, _) => IconButton(
              tooltip: facade.hasModel ? 'AI model đã sẵn sàng' : 'Import AI model',
              icon: Icon(facade.hasModel ? Icons.memory : Icons.download_for_offline_outlined),
              onPressed: () async {
                final success = await facade.importModelFromUser();
                if (!context.mounted) return;
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text(success ? 'AI local đã sẵn sàng.' : 'Chưa import được model .gguf.')),
                );
              },
            ),
          ),
              ...
              child: Padding(
                ...
                child: Row(
                  ...
                  children: [
                    Expanded(
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: 'Hỏi I2U về từ vựng, ngữ pháp...',
                          filled: true,
                    ...
                    IconButton.filled(
                      tooltip: 'Gửi',
                      onPressed: facade.isChatLoading ? null : () => _send(facade),
  ...
  Widget build(BuildContext context) => Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: const [
              Icon(Icons.auto_awesome, size: 48, color: Color(0xFFB388FF)),
              SizedBox(height: 16),
              Text('Trợ lý học tập I2U', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              SizedBox(height: 8),
              Text('Đặt câu hỏi về tiếng Anh hoặc nội dung bạn đang học.', textAlign: TextAlign.center, style: TextStyle(color: Colors.white70)),
            ],
          ),
        ),
      );
