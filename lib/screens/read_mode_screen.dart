...
// VipSound - Chế độ ĐỌC (Refactored: Text First, Tools on Demand)
...
class _ReadModeScreenState extends State<ReadModeScreen>
    ...
    return Consumer2<TextProvider, PlayerProvider>(
      builder: (context, textProvider, player, child) {
        ...
        return Column(
          children: [
            // ========== TOP BAR (Mỏng) ==========
            _buildMinimalTopBar(context, textProvider),

            // ========== MAIN TEXT CONTENT (90%) ==========
            Expanded(
              child: GestureDetector(
                onTap: _removeFloatingMenu, // Tap ngoài để đóng menu
                child: _buildTextContent(context, textProvider, player),
              ),
            ),

            // ========== BOTTOM BAR (Mỏng) ==========
            _buildMinimalBottomBar(context, textProvider),
          ],
        );
      },
    );
  ...
  Widget _buildMinimalTopBar(BuildContext context, TextProvider textProvider) {
    return Container(
      ...
      child: Row(
        children: [
          ...
          // Settings Button
          GestureDetector(
            onTap: () => _showSettingsSheet(context, textProvider),
            child: Container(
      ...
      child: SafeArea(
        ...
        child: Row(
          ...
          children: [
            ...
            _BottomIcon(
              icon: Icons.palette,
              isActive: textProvider.colorMode != ColorMode.none,
            ...
            _BottomIcon(
              icon: Icons.bookmark,
              badge: textProvider.segments.isNotEmpty
  ...
  Widget _buildTextLine(
    ...
    return GestureDetector(
      ...
      onLongPress: () {
        _showLineActionsMenu(context, textProvider, player, index, line);
      },
    ...
    return Wrap(
      ...
      children: words.asMap().entries.map((entry) {
        ...
        return GestureDetector(
          ...
          onLongPress: () {
            _showWordActionsMenu(
                context, textProvider, word, lineIndex, wordIndex);
          },
          child: Container(
  ...
  // ==================== FLOATING TEXT ACTIONS (Popup khi select) ====================
  ...
  void _handleTextSelection(
    ...
    if (selection.baseOffset != selection.extentOffset) {
      ...
      // Show floating menu
      _showFloatingTextActions(context, textProvider, selectedText);
    }
  }

  void _showFloatingTextActions(
    BuildContext context,
    TextProvider textProvider,
    String selectedText,
  ) {
    ...
    _floatingMenuOverlay = OverlayEntry(
      builder: (context) => Positioned(
        bottom: 100, // Hiện phía trên bottom bar
        left: 16,
        right: 16,
        child: Material(
  ...
  // ==================== SETTINGS SHEET (Chứa TTS Controls + Legend) ====================
            ...
            children: [
                ...
                children: [
                  ...
                  Expanded(
                    child: Slider(
                      ...
                      activeColor: const Color(0xFF2196F3),
                      onChanged: (value) {
                        textProvider.setTtsSpeed(value);
                        setModalState(() {});
                      },
                    ),
                  ),
              ...
              Wrap(
                ...
                children: ColorMode.values.map((mode) {
                  ...
                  return GestureDetector(
                    onTap: () {
                      textProvider.setColorMode(mode);
                      setModalState(() {});
                    },
                    child: Container(
              ...
              // Legend (if color mode is active)
              if (textProvider.colorMode != ColorMode.none) ...[
    ...
    switch (mode) {
      ...
      case ColorMode.wordType:
        return WordType.values.where((t) => t != WordType.unknown).map((type) {
          return _LegendChip(color: type.color, label: type.labelVi);
        }).toList();
      case ColorMode.cefrLevel:
        return CEFRLevel.values
            .where((l) => l != CEFRLevel.unknown)
            .map((level) {
          return _LegendChip(color: level.color, label: level.shortLabel);
        }).toList();
      case ColorMode.difficulty:
        return DifficultyLevel.values.map((level) {
          return _LegendChip(color: level.color, label: level.label);
        }).toList();
    }
  }

  // ==================== LINE ACTIONS MENU (Long press on line) ====================

  void _showLineActionsMenu(
    BuildContext context,
    TextProvider textProvider,
    PlayerProvider player,
    int index,
  ...
  // ==================== WORD ACTIONS MENU (Long press on word) ====================

  void _showWordActionsMenu(
    BuildContext context,
    TextProvider textProvider,
    AnalyzedWord word,
    int lineIndex,
        ...
        builder: (context, setModalState) => Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // Difficulty
              const Text('Độ khó:', style: TextStyle(color: Colors.white70)),
              const SizedBox(height: 8),
              Wrap(
                spacing: 8,
