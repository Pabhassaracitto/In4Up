...
namespace UltraTimeStretch
{
    ...
    {
        PFFFT_Setup *setup_;
        float *work_; // aligned buffer cho PFFFT
        int size_;

    public:
        FFTProcessor(int size) : size_(size)
        {
            // PFFFT_REAL: Tối ưu cho tín hiệu thực (nhanh hơn 2x so với Complex FFT)
        ...
        // Forward: Real Input -> Complex Output (Canonical format)
        void forward(const float *input, float *output)
        {
            pffft_transform_ordered(setup_, input, output, work_, PFFFT_FORWARD);
        }

        // Inverse: Complex Input -> Real Output
        void inverse(const float *input, float *output)
        {
            pffft_transform_ordered(setup_, input, output, work_, PFFFT_BACKWARD);

            // PFFFT inverse cần scale nếu cần thiết (thường là 1/N)
            // Tùy thuộc vào implementation của PhaseVocoder có handle việc này chưa
            float scale = 1.0f / size_;
            // PFFFT_SIMD_SUPPORT check nếu cần tối ưu vòng lặp này
