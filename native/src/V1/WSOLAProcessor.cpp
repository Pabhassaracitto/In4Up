...
namespace UltraTimeStretch
{
    ...
    WSOLAProcessor::WSOLAProcessor(int frameSize, int sampleRate)
        : frameSize_(frameSize), overlapSize_(frameSize / 2), sampleRate_(sampleRate), ratio_(1.0f), inputPosition_(0.0f)
    {
    ...
    int WSOLAProcessor::findBestOffset(const float *current, const float *previous, int searchRange)
    {
        ...
        {
            int readPos = overlapSize_ + offset;
            if (readPos < 0 || readPos + correlationLength > frameSize_)
                continue;

            float correlation = computeCorrelation(
                previous + readPos,
                current, // current frame's overlap region
                correlationLength);
