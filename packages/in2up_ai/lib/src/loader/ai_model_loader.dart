...
class AiModelConfig {
  ...
  static const String _prefKeyModelPath = 'in2up_ai_model_path';
  static const String _prefKeyModelSource = 'in2up_ai_model_source';
...
class AiModelLoader {
  ...
  Future<ModelLoadResult> _checkPreviouslyImported() async {
    try {
      ...
      // Verify file vẫn còn tồn tại và có header GGUF hợp lệ.
  ...
  Future<ModelLoadResult> importModelFromUser() async {
    try {
      ...
      final validationError = await _validateGguf(sourceFile);
      if (validationError != null) {
        return ModelLoadResult(
          success: false,
          source: ModelSource.none,
