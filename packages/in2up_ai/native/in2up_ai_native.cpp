...
static std::string generate_text(In2upAiContext * ai, const char * prompt, int max_tokens, float temperature) {
...
extern "C" in2up_ai_handle in2up_ai_create(const char * model_path, int n_ctx, int n_threads) {
    if (!model_path || !*model_path) return nullptr;
    llama_backend_init();

    auto * ai = new In2upAiContext();
...
extern "C" int in2up_ai_generate(in2up_ai_handle handle, const char * prompt, int max_tokens, float temperature, char ** output) {
    ...
    auto * ai = static_cast<In2upAiContext *>(handle);
...
extern "C" void in2up_ai_free_string(char * value) { std::free(value); }

extern "C" void in2up_ai_destroy(in2up_ai_handle handle) {
    auto * ai = static_cast<In2upAiContext *>(handle);
    if (!ai) return;
