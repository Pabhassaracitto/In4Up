...
class AiEngineGemma implements AiEngine {
  ...
  static String _mockInference(String prompt) {
    if (prompt.contains('in4up_WRITE_REVIEW')) {
      return _mockWriteReview(prompt);
    }
    if (prompt.contains('in4up_REWRITE_REVIEW')) {
  ...
  static String _mockWriteReview(String prompt) {
    ...
    if (totalScore >= 90) {
      summary =
          'Bài làm rất sát câu gốc, bạn đang giữ được cả ý và trật tự câu khá tốt.';
      actions
          .add('Có thể chuyển sang câu mới hoặc tăng tốc độ luyện một chút.');
      topics.add('High Accuracy');
    } else if (totalScore >= 75) {
      summary =
          'Bài làm khá ổn, bạn đã nắm phần lớn nội dung nhưng vẫn còn vài điểm lệch nhỏ.';
      actions.add('Làm lại thêm 1 vòng ngay cùng câu để khóa phản xạ đúng.');
      topics.add('Stable Recall');
    } else if (totalScore >= 55) {
      summary =
          'Bạn đã bắt được khung câu, nhưng độ chính xác chưa ổn định ở mọi phần.';
      actions.add(
          'Chia câu thành các cụm ngắn rồi chép lại từng cụm trước khi ghép cả câu.');
      topics.add('Needs Reinforcement');
    } else {
      summary =
          'Bài làm còn lệch khá nhiều so với câu gốc, nên giảm tải và luyện lại theo cụm nhỏ.';
      actions.add(
          'Nghe chậm hơn hoặc dùng TTS để bám âm tốt hơn trước khi chép lại.');
    ...
    if (extra.isNotEmpty && extra != 'none') {
      actions
          .add('Tránh thêm ý đoán; chỉ chép những gì thật sự nghe/nhớ được.');
    ...
    if (spellingScore < 70) {
      actions
          .add('Mở đáp án và chép đúng mặt chữ 1 vòng để giảm lỗi chính tả.');
      topics.add('Spelling');
    }

    final grammar =
        _mockGrammarFromSentence(expected.isNotEmpty ? expected : actual);
  ...
  static String _mockRewriteReview(String prompt) {
    ...
    if (totalScore >= 85) {
      summary =
          'Bài viết lại giữ ý khá tốt và đã có dấu hiệu diễn đạt theo cách riêng.';
      ...
      actions.add(
          'Giữ lại 3 từ khóa cốt lõi rồi viết lại chỉ quanh các từ khóa đó.');
      topics.add('Missing Core Idea');
    } else if (paraphraseScore < 40) {
      summary =
          'Bạn đang bám quá sát câu gốc, chưa thật sự viết lại bằng câu khác.';
      ...
      summary =
          'Bài viết lại ở mức khá, có thể tiếp tục tinh chỉnh độ tự nhiên và độ gọn.';
      actions.add(
          'Làm lại thêm 1 vòng, ưu tiên thay từ/cụm thay vì chép khung cũ.');
    ...
    if (kept.isNotEmpty && kept != 'none') {
      actions.add(
          'Bạn đã giữ được các từ khóa: $kept. Hãy giữ chúng nhưng đổi khung câu hơn nữa.');
      topics.add('Keyword Retention');
    }

    final grammar =
        _mockGrammarFromSentence(actual.isNotEmpty ? actual : expected);
  ...
  static String _mockSummaryReview(String prompt) {
    ...
    if (totalScore >= 85) {
      ...
      actions.add(
          'Thử rút xuống còn gọn hơn một chút nhưng vẫn giữ 2 từ khóa quan trọng nhất.');
      topics.add('Concise & Accurate');
    } else if (contentScore < 45) {
      summary = 'Bản tóm tắt đang mất nhiều ý cốt lõi.';
      actions.add(
          'Giữ lại 2–3 từ khóa trọng tâm rồi viết lại một câu ngắn quanh các từ đó.');
      topics.add('Lost Core Meaning');
    } else if (brevityScore < 45) {
      summary = 'Bạn giữ ý khá tốt nhưng chưa tóm gọn đủ, vẫn còn quá dài.';
      actions
          .add('Lược bỏ cụm phụ, trạng từ hoặc giải thích phụ để câu sắc hơn.');
      topics.add('Too Long');
    } else if (grammarScore < 45) {
      summary =
          'Bản tóm tắt khá ngắn nhưng hình dáng câu chưa đủ rõ và tự nhiên.';
      actions.add(
          'Viết thành một câu hoàn chỉnh có động từ chính và kết thúc bằng dấu câu.');
      topics.add('Sentence Shape');
    } else {
      summary =
          'Bản tóm tắt ở mức ổn, cần tinh chỉnh thêm để vừa ngắn vừa bén ý hơn.';
      actions.add(
          'Soát lại xem có thể bỏ thêm từ nào mà không làm mất ý chính không.');
    ...
    if (kept.isNotEmpty && kept != 'none') {
      actions.add(
          'Bạn đã giữ được các từ khóa: $kept. Hãy dùng chúng làm trục khi rút gọn.');
      topics.add('Keyword Retention');
    }
    if (compression.isNotEmpty) {
      actions.add(
          'Tỉ lệ hiện tại: $compression. Cân bằng giữa độ ngắn và độ đủ ý.');
    }
  ...
  static Map<String, dynamic> _mockGrammarFromSentence(String sentence) {
    ...
    return {
      ...
      'explanation_vi':
          'Đây là phân tích ngắn theo kiểu beta để hỗ trợ người học nhìn lại cấu trúc câu khi luyện viết.',
    };
  }
