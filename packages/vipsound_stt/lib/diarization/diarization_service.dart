// VipSound v11.0 — Diarization interface + Heuristic Sprint 1
...
class HeuristicDiarizationService implements DiarizationService {
  ...
  Future<List<SpeakerAnnotation>> diarize(SttResult input) async {
    ...
    for (var i = 1; i < input.segments.length; i++) {
      ...
      final isShortReply =
          curr.text.trim().split(RegExp(r'\s+')).length <= 5;

      final shouldChange =
          gap >= silenceGapSec || (isQuestion && isShortReply);
      ...
      final conf = gap >= silenceGapSec
          ? 0.9
          : (isQuestion && isShortReply ? 0.7 : 0.6);
...
class MeetilyRustDiarizationService implements DiarizationService {
  ...
  Future<List<SpeakerAnnotation>> diarize(SttResult input) async {
    debugPrint(
        '[MeetilyRust] Bridge chưa sẵn sàng → fallback Heuristic');
