// VipSound v11.0 — Sidecar .spk.json (cache render, KHÔNG phải DB)
...
class SpeakerSidecar {
  ...
  static String getSidecarPath(String lrcPath) =>
      lrcPath.replaceAll(
  ...
  static Future<Map<String, int>> loadSpeakerMap(
      String lrcPath) async {
    final file = File(getSidecarPath(lrcPath));
    ...
    try {
      ...
      for (final item in list) {
        final a = SpeakerAnnotation.fromJson(
      ...
      debugPrint(
          '[SpeakerSidecar] Loaded ${map.length ~/ 2} annotations '
          'from ${file.path}');
      return map;
    } catch (e) {
