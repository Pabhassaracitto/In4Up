...
import 'package:in4up/features/translation/engines/hymt_slot.dart';

void main() {
  group('HyMtSlot', () {
    test('first caller acquires immediately; release frees the slot', () async {
      final slot = HyMtSlot(maxWait: const Duration(milliseconds: 50));
    ...
    test('concurrent callers serialize: never more than one holder', () async {
      final slot = HyMtSlot(maxWait: const Duration(seconds: 2));
      var holders = 0;
      var maxHolders = 0;
    ...
    test('a waiter that times out is NOT handed the slot later', () async {
      final slot = HyMtSlot(maxWait: const Duration(milliseconds: 30));
      expect(await slot.acquire(), isTrue);
      final timedOut = slot.acquire();
      await Future<void>.delayed(const Duration(milliseconds: 60));
      expect(await timedOut, isFalse);
      // Holder trả slot → slot TRỐNG (không trao cho waiter đã hết hạn).
      slot.release();
      expect(slot.isHeld, isFalse);
      expect(slot.pendingCount, 0);
    });

    test('handoff under load keeps FIFO order for 3 waiters', () async {
      final slot = HyMtSlot(maxWait: const Duration(seconds: 2));
      final order = <String>[];
      expect(await slot.acquire(), isTrue);

      final second = slot.acquire().then((ok) async {
        expect(ok, isTrue);
        order.add('second');
        slot.release();
      });
      final third = slot.acquire().then((ok) async {
        expect(ok, isTrue);
        order.add('third');
        slot.release();
      });
      ...
      expect(order, <String>['first-done', 'second', 'third']);
      expect(slot.isHeld, isFalse);
    });
  });
}
