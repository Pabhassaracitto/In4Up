...
//
// Fixture dùng JSON thuần (đúng như dữ liệu thật trong Hive box
// 'vocabulary_v2' đang lưu jsonEncode) — WordEntry.fromJson tự dựng
// SkillReviewData bên trong. Test KHÔNG nhắc tên SkillReviewData:
// tổ hợp import/export từng làm CI gãy (postmortem bisect B1–B7, T3–T5a
// — xem ghi chú trong word_entry_migrator.dart).
...
WordEntry _entry(
  ...
  return WordEntry.fromJson(<String, dynamic>{
    'id': id,
    'word': word,
    ...
    'listenData': l,
    'readData': r,
    'lastReviewed': DateTime.utc(2026, 2, 20, 9).toIso8601String(),
    'createdAt': DateTime.utc(2026, 1, 10).toIso8601String(),
    'updatedAt': DateTime.utc(2026, 2, 20, 9).toIso8601String(),
...
void main() {
  ...
  group('WordEntryMigrator — DoD Task 3', () {
    test('1:1 KHÔNG MẤT TỪ NÀO — mọi id cũ thành unitId (giữ FK lịch sử)', () {
      ...
      expect(
        result.units.map((unit) => unit.unitId).toSet(),
        ...
        [
          _entry(
            ...
            u: _skillJson(
              ...
              interval: 3,
              repetitions: 1,
              nextReview: DateTime.utc(2026, 9, 1).toIso8601String(),
            ),
            l: _skillJson(
              easeFactor: 1.8,
              interval: 21,
              repetitions: 4,
              nextReview: DateTime.utc(2026, 8, 25).toIso8601String(),
            ),
            r: _skillJson(totalReviews: 0), // chưa từng review lần nào
          ),
        ],
        now: at,
      );
    ...
    test('Legacy JSON (định dạng cũ không understandData) migrate được', () {
    ...
    test('kind mapping: word/phrase/sentence/paragraph đúng loại unit', () {
      final result = WordEntryMigrator.migrate(
        [
          _entry('k1', 'bank'),
          _entry('k2', 'look up', vocabType: 'phrase'),
          _entry('k3', 'I look up.', vocabType: 'sentence'),
          _entry('k4', 'A paragraph.', vocabType: 'paragraph'),
        ],
        now: at,
      );
