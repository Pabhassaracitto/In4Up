...
void main() {
  ...
  group('WordEntryMigrator — DoD Task 3', () {
    ...
    test('VocabContext ⇒ Evidence: đủ số lượng, đúng FK, hash/locator reopen được',
        () {
      ...
      final result =
          WordEntryMigrator.migrate([_entry('v_9_0', 'bank', contexts: contexts)], now: at);
    ...
    test('DUE DATE KHÔNG ĐỔI: snapshot byte-đồng nhất nextReview cũ cả 3 skill',
        () {
      final u = _skill(
          easeFactor: 2.4,
          interval: 3,
      ...
      final result =
          WordEntryMigrator.migrate([_entry('v_due_0', 'bank', u: u, l: l, r: r)], now: at);
    ...
    test('Trường không có trong schema v1 được LIỆT KÊ (không mất âm thầm)', () {
      ...
      expect(result.report.fieldsNotRepresentedInV1, containsAll(['tags', 'topic', 'meaning']));
